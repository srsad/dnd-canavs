...
- данные комнаты (холст, чат, кубики, до 50 снимков в `canvasHistory`) хранятся в БД; **активные серверные сессии подключения** — в памяти процесса: после рестарта бэкенда нужен повторный `join`; при новом `join` с той же ролью старая websocket-сессия снимается (evict)
- нет отдельной ACL кроме ролей `gm` / `player` и поля `controlledByParticipantId` у фишек
- нет экспорта/импорта комнат и **полного** журнала действий (есть только ограниченная история снимков холста у мастера, без аудита чата/кубиков/движений по шагам)
